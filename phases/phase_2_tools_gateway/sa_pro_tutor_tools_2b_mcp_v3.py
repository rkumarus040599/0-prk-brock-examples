...
# ---------------------------------------------------------------------------
# AgentCore Identity configuration (placeholder for future)
# ---------------------------------------------------------------------------
...
#
# In a later iteration, we will replace MCP_BEARER_TOKEN with AgentCore
# Identity-based outbound auth (for example, via @requires_access_token).[web:340]
# For now, we keep MCP_BEARER_TOKEN as a local/dev + runtime fallback.
...
# Env-based bearer for Gateway (local/dev + simple runtime config)
...
def _get_gateway_access_token() -> str:
    ...
    Today:
      - Uses MCP_BEARER_TOKEN from the environment (local/dev and simple runtime).

    Next iteration:
      - Replace this with AgentCore Identity outbound auth using the configured
        OAuth2 credential provider and scope.[web:340]
    ...
    raise RuntimeError(
        ...
        "Set MCP_GATEWAY_BEARER_TOKEN in the environment for now. "
        "Later, this will be provided by AgentCore Identity."
    )
