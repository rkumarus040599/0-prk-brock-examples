# TODO (Phase 2b-mcp-v4):
# Replace env-based MCP_BEARER_TOKEN with AgentCore Identity outbound auth.
# This function should:
#   - Use the configured OAuth2 credential provider:
#       IDENTITY_OAUTH2_CREDENTIAL_PROVIDER_ARN
#       IDENTITY_OAUTH2_SCOPE
#   - Obtain an OAuth2 access token at runtime via AgentCore Identity.
#   - Return the token string so existing callers (MCP client + HTTP helper)
...
- Output is always: {"result": {"text": "<answer as plain text>"}}  # logical shape
...
# NEW: Identity decorator import for outbound OAuth2 (M2M)
from bedrock_agentcore.identity.auth import requires_access_token  # type: ignore

# Local dev only; AgentCore Runtime will use Identity / outbound auth later
load_dotenv()
...
#
# We will use Identity outbound auth (M2M) to get a bearer token at runtime.[web:340]
...
# Derive provider_name from ARN (last path component)
IDENTITY_OAUTH2_PROVIDER_NAME = IDENTITY_OAUTH2_CREDENTIAL_PROVIDER_ARN.rsplit("/", 1)[-1]

# Env-based bearer for Gateway (local/dev override or fallback)
MCP_BEARER_TOKEN = os.getenv("MCP_GATEWAY_BEARER_TOKEN", "")

logger.info(f"MCP token present in env: {bool(MCP_BEARER_TOKEN)}")
...
# ---------------------------------------------------------------------------
# Helper to obtain an access token for Gateway calls (v4 with Identity)
# ---------------------------------------------------------------------------

@requires_access_token(
    provider_name=IDENTITY_OAUTH2_PROVIDER_NAME,
    scopes=[IDENTITY_OAUTH2_SCOPE],
    auth_flow="M2M",
)
async def _identity_get_gateway_token(*, access_token: str) -> str:
    """
    Identity-integrated helper.

    AgentCore Identity will:
      - Use the configured OAuth2 credential provider (Cognito client).
      - Perform the M2M (client credentials) flow as needed.
      - Inject a valid access_token here when called.[web:340]

    We just return the access_token string to the caller.
    """
    return access_token
...
def _get_gateway_access_token() -> str:
    ...
    Behavior:
      - If MCP_BEARER_TOKEN is set in env, use it (manual override / debug).
      - Otherwise, obtain a token from AgentCore Identity using the
        configured OAuth2 credential provider (M2M flow).

    This keeps v3's behavior available via env, while making Identity
    the default source in v4.
    """
    # 1) Manual env override for debugging and recovery
    if MCP_BEARER_TOKEN:
        return MCP_BEARER_TOKEN

    # 2) Normal path: use AgentCore Identity outbound OAuth (M2M)
    try:
        # The decorator is async; we call it via asyncio.run from sync code.
        # AgentCore Runtime runs this inside its own event loop-aware environment,
        # so this pattern is acceptable for short, bounded token retrieval.
        import asyncio

        token: str = asyncio.run(_identity_get_gateway_token())
        if not token:
            raise RuntimeError("AgentCore Identity returned an empty access token.")
        return token
    except Exception as exc:
        raise RuntimeError(
            f"Failed to obtain access token from AgentCore Identity: {exc}"
        ) from exc
